...
export const ProductModel = {
  ...
  create: async (productData) => {
    ...
    if (id_editorial) {
      await pool.query(
        "INSERT INTO Producto_Editorial (id_producto, id_editorial) VALUES (?, ?)",
        [id_producto, id_editorial]
      );
    }
  ...
  catalog: async ({
    ...
    if (onlyOffers) {
      whereClauses.push(`o.precio_oferta IS NOT NULL`);
    }

    const whereSQL = whereClauses.length ? `WHERE ${whereClauses.join(" AND ")}` : "";

    let orderSQL = "ORDER BY p.id_producto DESC"; // newest
    ...
    if (sort === "offersNewest") {
      orderSQL = "ORDER BY (o.precio_oferta IS NOT NULL) DESC, p.id_producto DESC";
    }
